...
var ticketMethods = {
  ...
  getIndexToThrowInto: function(ticket:any,value:any){
    ...
    for(var row=0;row<3;row++){
      if(ticket[row][columnToObserve] == 0 && ticket[row].filter(function(n:any){ return n!=0; }).length < 5){
        indices = [0,0];
        indices[0] = row;
  ...
  sortColumns: function(ticket:any){
  ...
  removeValuesUsed: function(ticket:any){
